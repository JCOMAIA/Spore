# -*- coding: utf-8 -*-
"""Colonizacao de habitat ASSINADO â€” nunca de maquina nao autorizada.
...
Sem assinatura humana nao ha colonizacao â€” essa e a fronteira do experimento."""
...
def build_plan(habitat_id):
    ...
    if h.get("status") not in ("approved", "home"):
        print("habitat %r status=%r â€” colonizacao EXIGE 'approved' (assinatura humana)."
              % (habitat_id, h.get("status")))
        return None
    if not h.get("host"):
        print("habitat %r sem 'host' â€” edite habitats.json depois de assinar." % habitat_id)
        return None
    ...
    lines = [
        "# Plano de deploy â€” %s (gerado %s)" % (habitat_id, time.strftime("%Y-%m-%d %H:%M")),
        "",
        "```bash",
...
def main(argv=None):
    ...
    if "--exec" in argv:
        ...
        if ans.strip().upper() == "ENVIAR":
            ...
            print("[colonize] rsync terminou com codigo %s â€” siga os passos 2-4 do plano." % rc)
    return 0
