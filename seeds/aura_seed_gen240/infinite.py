...
e kill switch manual - crie o arquivo guard/STOP e o loop para no proximo ciclo.
"""
...
def _hb(fase):
    """Pulso: fase do ciclo (ciclo/vivo) para o painel distinguir
    'EM CICLO' (trabalhando, pode demorar) de 'SILENCIO' (parada)."""
    try:
        with open(os.path.join(ROOT, "guard", "heartbeat.json"), "w",
                  encoding="utf-8") as f:
            json.dump({"ts": time.time(), "fase": fase}, f)
    except OSError:
        pass
...
def cycle(args, limits, rng):
    ...
    # v2: o campeao e RE-MEDIDO a todo ciclo - comparacao simetrica,
    # a barra congelada por sorteio morreu (audit A1).
    r = evaluator.run(champ_genome, client=make_client(champ_genome),
                      ...
                      latency_slow_s=float(limits.cfg.get("latency_slow_s", 0.0)),
                      latency_penalty_max=float(limits.cfg.get("latency_penalty_max", 0.0)))
    ...
    if not dry:
        time.sleep(float(os.environ.get("AURA_EVAL_GAP_S", "3.0")))  # respira (throttle)

    gen_n = current_gen_number() + 1
    mutated_genome, mutated_state, rec = mutator.mutate(
        champ_genome, champ_state, rng,
        client=None if dry else make_client(champ_genome))
    ...
    if verdict == "death" and rec.get("op") == "routing" and rec.get("after"):
        # Aprendizado de evitacao: alimento que engasga (429) ou nao tem credito
        # (402) sai do pool do campeao. Nao e julgamento de qualidade - e acesso.
        # Auditavel em guard/avoids.jsonl; a semente herda a aversao.
        errs = " | ".join(str(v) for k, v in (cr.get("breakdown") or {}).items()
                           if str(k).endswith("__err"))
        motivo = "429" if "429" in errs else ("402" if "402" in errs else None)
        if motivo:
            pool = champ_genome.setdefault("routing", {}).setdefault("decider_models", [])
            if rec["after"] in pool:
                pool.remove(rec["after"])
                _save_json(os.path.join(best, "genome.json"), champ_genome)
                with open(os.path.join(ROOT, "guard", "avoids.jsonl"), "a",
                          encoding="utf-8") as f:
                    f.write(json.dumps({"ts": time.time(), "modelo": rec["after"],
                                        "motivo": motivo + " (gen %d)" % gen_n},
                                       ensure_ascii=False) + "\n")
                print("[evitar] alimento envenenado removido do pool: %s" % rec["after"])
    if verdict == "survive":
    ...
    try:
        from observacao import descobridor
        n_ev = descobridor.observe(entry)
        if n_ev:
            print("[descobridor] %d evento(s) registrado(s) na gen %d" % (n_ev, gen_n))
    except Exception as e:
        print("[descobridor] falhou (o ciclo continua): %s" % str(e)[:120])
...
def summary_selftest():
    ...
    print("selftest OK - o ciclo N+1 funciona (medir -> mutar -> reproduzir -> podar -> registrar).")
    return 0
...
def main(argv=None):
    ...
    try:
        from guard import integrity
        ok, msgs = integrity.check()
        if not ok:
            with open(os.path.join(ROOT, "guard", "STOP"), "w", encoding="utf-8") as f:
                f.write("integridade do kernel comprometida")
            print("[guard] INTEGRIDADE DO KERNEL FALHOU - STOP gravado:")
            for m in msgs:
                print("  -", m)
            print("  (depois de sync legitimo: python3 guard/integrity.py --init; rm guard/STOP)")
            return 1
    except Exception as e:
        print("[guard] integridade indisponivel (%s) - seguindo" % e)
    ap = argparse.ArgumentParser(description="Aura Infinite - ciclo N+1")
    ...
    ap.add_argument("--hands-every", type=int, default=0,
                    help="turno de agencia a cada N geracoes; 0=desligado")
    ...
    if args.selftest:
        ...
        args.cycles = 12
        args.sleep = 0
        print("=== AURA INFINITE - selftest (dry: paisagem sintetica, zero API) ===")
        rng = random.Random(7)
    else:
        rng = random.Random()

    limits = Limits(ROOT)
    # lock do corpo: UMA vida por diretorio (duas vidas = race de gen/best/gasto)
    from guard import bodylock
    ok_l, _fh, msg_l = bodylock.acquire(ROOT)
    if not ok_l:
        print("[guard] " + msg_l)
        print("       (ciclos manuais: sudo systemctl stop aura-infinite primeiro)")
        return 1
    os.environ.setdefault("AURA_LOCK_HELD", "1")
    init_genesis()
    ...
    hands_every = int(getattr(args, "hands_every", 0) or 0)
    for i in range(args.cycles):
        ...
        if spor_every and not dry_mode and gen_n and gen_n % spor_every == 0:
            try:
                import hashlib
                gp = os.path.join(LINEAGE, "best", "genome.json")
                sha = hashlib.sha256(open(gp, "rb").read()).hexdigest()
                lp = os.path.join(ROOT, "guard", "last_seed.sha")
                last = open(lp).read().strip() if os.path.exists(lp) else ""
                if sha == last:
                    print("[esporulacao] campeao identico ao ultimo publicado — "
                          "pulando (gen %d)" % gen_n)
                else:
        ...
        if hands_every and not dry_mode and gen_n and gen_n % hands_every == 0:
            try:
                from hands import shift as _hands
                _hands.run_shift(gen=gen_n)
            except Exception as e:
                print("[agencia] falhou (o ciclo continua): %s" % e)
